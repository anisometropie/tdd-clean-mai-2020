...
describe('zip code selector', () => {
    ...
    const courcellesBuilding = {id: '456abc', address: '4 rue de Courcelles 75017 Paris'};
    ...
    it('should get distinct zip codes from several available buildings', () => {
        agencyGateway.buildings = new Map([[PARIS, [fochBuilding, courcellesBuilding]]])
        retrieveBuildings(PARIS);
        expect(getZipCodes(store.getState())).toEqual(['75008', '75017']);
    });

    it('should get distinct zip codes from several available buildings', () => {
        agencyGateway.buildings = new Map([[PARIS, [fochBuilding, foch2Building, courcellesBuilding]]])
        retrieveBuildings(PARIS);
        expect(getZipCodes(store.getState())).toEqual(['75008', '75017']);
    });

    it('should not get any zip code when address is malformed', () => {
        agencyGateway.buildings = new Map([[PARIS, [{...fochBuilding, address: 'Paris'}]]]);
        retrieveBuildings(PARIS);
        expect(getZipCodes(store.getState())).toEqual([]);
    });
